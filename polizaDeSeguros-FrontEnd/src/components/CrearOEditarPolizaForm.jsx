...
const CreateOrEditPoliza = () => {
  ...
  const cargarPoliza = async (idPoliza) => {
    if (!token) {
      setSnackbarMessage('No se encontró el token. Por favor inicie sesión nuevamente.');
  ...
  return (
    <>
      <AppBar position="static" color="primary">
        <Toolbar>
          <IconButton edge="start" color="inherit" onClick={() => navigate(id ? '/obtenerPolizas' : '/home')} aria-label="volver">
            <ArrowBackIcon />
          </IconButton>
        ...
        <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          ...
          <Box sx={{ flex: 1 }}>
            <NumericFormat
              ...
              fixedDecimalScale
              allowNegative={false}
              customInput={TextField}  // usa TextField como input base
              fullWidth
              required
            />
          </Box>
